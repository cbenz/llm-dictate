...
- the user configures a keyboard shortcut (e.g. `mod+backspace`) in their desktop environment to run `./src/llm-dictate toggle`
- when the shortcut is pressed, the notification "🎤 Recording..." appears
- the user speaks for as long as they want
- the recording can be canceled by running `./src/llm-dictate cancel` (e.g. bound to another shortcut or run manually)
